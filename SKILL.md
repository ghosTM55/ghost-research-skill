...
| 开放式市场或竞品研究 | 轻量中英文搜索 → 筛选 → 正文读取 | 核心问题有可追溯证据，缺口明确 |
...
> 你需要联网获取信息，使用当前环境可用的搜索、网页读取、浏览器或终端检索工具，并遵守上文的资料访问、安全边界与停止条件。指定公开 URL 直接读取；开放研究优先轻量中英文搜索；已知动态、登录、交互页面直接使用获授权的浏览器会话。仅围绕真实证据缺口补搜，不反复调用失败路径。一手来源优于二手来源：官方博客 > 权威媒体原创报道 > 转载/聚合。如果研究对象涉及学术概念、算法、AI 模型或技术范式，应检索 arXiv / 论文来源。优先使用可用的 arxiv skill 或官方 arXiv API；构造查询时正确 URL encode，原始来源 URL 保留原样。
...
2. **数据底盘**：优先使用最新可得报告期的数据，同时保留必要的历史趋势。过旧数据只能作为背景，不应替代当前判断。多个数据源冲突时，说明口径、时间、采集方式和可能偏差。
3. **结构拆解**：分别看需求端、供给端、渠道端、价格/成本端、替代方案和约束条件。不要只回答“有没有市场”，要回答市场如何被组织、利润在哪里、摩擦在哪里。
4. **主体地图**：识别终端用户/客户、采购方、渠道商、分销商、供应商、监管者、平台、协会、替代方案和影响者。市场碎片化时，不假设存在单一集中买方；应分析谁拥有采购权、渠道控制权和实际交易能力。
...
1. **Evidence refresh**：读取原文后，识别所有时间敏感事实、关键数据、公司/产品状态、监管/价格/市场变化和来源缺口；对会影响结论的事实重新查证。
2. **Boundary repair**：检查研究对象是否混入相邻但不同的概念、品类、统计口径或客户群；必要时拆成主市场、相邻市场和待验证假设。
3. **Argument restructuring**：删除“回答问题1/2/3”“对应你提出的问题”“下面我将”等问答痕迹，把素材重组为执行判断、证据链、结构分析、反证/盲点、行动建议。
4. **Evidence hierarchy**：明确区分已验证事实、基于证据的推断、假设、建议和下一步验证；不要用流畅文案掩盖证据不足。
5. **Report polish**：标题要结论化，段落要减少废话，表格要服务判断；不要在报告正文中暴露工具或 skill 自我署名，保留必要的方法论或来源署名即可。
...
这不是一份冷冰冰的咨询报告，也不是聊天问答记录。它应该是一份可进入 KBS 的专业研究报告：结论先行、证据清楚、判断克制、标题有含义。深度来自问题定义、证据密度和结构洞察，而不是堆字数或堆材料。

### 核心原则

**结论先行**：执行摘要和关键标题要表达判断，而不只是标注主题。标题应回答“这意味着什么”。

**证据分层**：把事实、推断、假设和建议分开。数据源冲突时写出冲突，不要把不确定性藏进顺滑文案里。

**结构优先**：研究报告的价值来自结构化解释：边界、演化、横向位置、机制、风险、行动路径，而不是材料堆砌。
...
- 问答残留：「回答问题1/2/3」、「对应你提出的问题」、「下面我将」
- 工具自我署名：「基于某某 skill 能力」这类把工具名写进报告正文的表达
- 空洞词：「赋能」、「抓手」、「打造闭环」
...
- [ ] 市场/机会类研究是否先定义边界、分类口径、相邻概念和排除项？
- [ ] 是否使用最新可得数据，并把旧数据放回历史背景或趋势分析，而不是当前事实？
- [ ] 是否把已验证事实、推断、假设、建议和下一步验证拆开？
- [ ] 是否删除问答残留和工具自我署名，让文档像专业研究报告而不是聊天记录？
- [ ] 对名单、公司、渠道和买方线索，是否标明证据等级和后续验证方式？
